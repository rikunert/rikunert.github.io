...
lead: Streets, squares and places in general tend to be named after influential people. As a consequence, place names offer a window into who shaped local cultures. In this post I use open street map data to visualise the influence of different historical figures in Germany.
---
...
Open street map is a volunteer based list of geographical entities including their coordinates and names. One can query the open street map data base with the Python module `overpy`. Before we write the function to do that, let's first import the modules we'll need for this project.
...
Please note that, as with many APIs, we can run into a rate limitation if we query too often. Waiting 30 seconds (`API_WAIT_TIME`) should fix that.

Let's try out the function see where Germany's first modern chancellor, Konrad Adenauer, gets honored.
...
Apparently, the first political leader of West Germany gets honored in place names all over, well, West Germany. Thirty years after the reunification of Germany he hardly has any place names after in the former East.
...
Under the hood the heat map is a 2D  histogram (`np.histogram2d()`). It's as if we lay a grid onto the map of Germany and count how many red dots appear in each cell. This 2D histogram then gets smoothed (`ndimage.filters.gaussian_filter()`), i.e. the values of nearby cells gets averaged. I plot the result (`ax.imshow()`).
...
Three hot spots of Adenauer commemoration are obvious: Berlin, Frankfurt, Ruhr region.
...
Heatmaps of place names today can be used to track cultural differences between different regions of Germany. However, the approach used in this post should has some caveats:

* The denser the population of a region, the more place names there are there in general and the more likely a hot spot is to appear in this region.
